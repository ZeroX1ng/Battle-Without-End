...
Last updated: 2026-06-21 (custom-name start route and EquipWindow lower detail utilization verified; shop-stock progression remains a product decision)
...
| P1-EQUIP-WINDOW-SPACE-UTILIZATION | P1 | 装备图标拥挤与下方空间利用 | Verified | `p1-equip-window-space-utilization.md` | Passed 2026-06-21: `assert:equip-window-space-utilization`, `assert:equip-window-detail-placement`, `assert:equip-window`, `assert:equip-window-bounds`, `assert:window-sprite-icons`, `npx tsc -b`; browser smoke confirms lower detail fills the scroll region and stat deltas use responsive multi-column layout |
| P1-LOOT-PANEL-LOG-ALIGNMENT | P1 | 当前地图统计与战斗日志对齐 | Verified | `p1-loot-panel-log-alignment.md` | Passed 2026-06-21: `assert:loot-panel-log-alignment`, `assert:responsive-layout`, `assert:battle-log-sticky-scroll`, `assert:monster-reward`, `npx tsc -b`; browser rect smoke at 1280x720/FHD/UHD |
| P2-TEST-SPEED-FLOATING-POPOVER | P2 | dev-only 调速浮动入口 | Verified | `p2-test-speed-floating-popover.md` | Passed 2026-06-21: `assert:test-speed-control`, `assert:game-loop`, `assert:responsive-layout`, `npx tsc -b`; browser rect smoke confirmed the trigger/popover stay left of the right tab rail and close without right-rail placeholder; release gating covered by static guard |
| P1-START-CUSTOM-PLAYER-NAME | P1 | 创建角色时自定义玩家名 | Verified | `p1-start-custom-player-name.md` | Passed 2026-06-21: `assert:start-custom-player-name` covers BeginScene -> SaveScene start routing, all four AS3 save slots, empty-slot custom name -> RaceScene -> main panel -> refresh/load |
...
| P1-EQUIP-WINDOW-SPACE-UTILIZATION | P1 | Equipment slot crowding and lower-space usage | Verified | `p1-equip-window-space-utilization.md` | Passed 2026-06-21: `assert:equip-window-space-utilization`, `assert:equip-window-detail-placement`, `assert:equip-window`, `assert:equip-window-bounds`, `assert:window-sprite-icons`, `npx tsc -b`; browser smoke confirms lower detail fills the scroll region and stat deltas use responsive multi-column layout |
| P1-LOOT-PANEL-LOG-ALIGNMENT | P1 | Current-map statistics alignment with battle log | Verified | `p1-loot-panel-log-alignment.md` | Passed 2026-06-21: `assert:loot-panel-log-alignment`, `assert:responsive-layout`, `assert:battle-log-sticky-scroll`, `assert:monster-reward`, `npx tsc -b`; browser rect smoke at 1280x720/FHD/UHD |
| P2-TEST-SPEED-FLOATING-POPOVER | P2 | Dev-only floating speed-control trigger | Verified | `p2-test-speed-floating-popover.md` | Passed 2026-06-21: `assert:test-speed-control`, `assert:game-loop`, `assert:responsive-layout`, `npx tsc -b`; browser rect smoke confirmed the trigger/popover stay left of the right tab rail and close without right-rail placeholder; release gating covered by static guard |
| P1-START-CUSTOM-PLAYER-NAME | P1 | Custom player name during character creation | Verified | `p1-start-custom-player-name.md` | Passed 2026-06-21: `assert:start-custom-player-name` covers BeginScene -> SaveScene start routing, all four AS3 save slots, empty-slot custom name -> RaceScene -> main panel -> refresh/load |
