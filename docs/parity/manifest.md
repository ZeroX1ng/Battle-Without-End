...
Last updated: 2026-06-08 (battle cadence verified; monster-info flicker, combat-power readout, and equipment tooltip-bounds remain routed)
...
**2026-06-08 重要提示：** `P0-BATTLE-TEMPO-CADENCE` 已由 `assert:battle-tempo-cadence` 守住 1x 前台单调度最多 1 个可见战斗 tick 的节奏边界，并完成 browser cadence smoke；`P1-MONSTER-INFO-ATTACK-FLICKER`、`P1-EQUIP-TOOLTIP-BOUNDS` 仍是玩家可见修复候选；`P1-COMBAT-POWER-EQUIPLESS-READOUT` 已确认 AS3/React 都是不含装备的基础 CP，后续若要改善真实强度表达，应作为 product override 单独决策，不能直接替换 AS3 `combatPower` 语义。
...
3. **试玩 follow-up 2026-06-08**：`P0-BATTLE-TEMPO-CADENCE` 已 Verified；下一张代码修复优先处理 `P1-MONSTER-INFO-ATTACK-FLICKER` 或 `P1-EQUIP-TOOLTIP-BOUNDS` 两个 UI 可见问题；`P1-COMBAT-POWER-EQUIPLESS-READOUT` 先做产品决策，不要直接改 AS3 `combatPower` 公式。
...
**2026-06-08 note:** `P0-BATTLE-TEMPO-CADENCE` is now guarded by `assert:battle-tempo-cadence`, which protects the 1x foreground rule that one scheduler pass emits at most one visible battle tick, and browser cadence smoke has passed. `P1-MONSTER-INFO-ATTACK-FLICKER` and `P1-EQUIP-TOOLTIP-BOUNDS` remain player-visible repair candidates. `P1-COMBAT-POWER-EQUIPLESS-READOUT` has AS3/React evidence showing that `combatPower` is base-only and excludes equipment; any stronger equipment-inclusive readout must be a product override and must not silently replace the AS3 `combatPower` semantics.
...
3. **Playtest follow-up 2026-06-08**: `P0-BATTLE-TEMPO-CADENCE` is Verified; next code repairs should choose one visible UI issue, either `P1-MONSTER-INFO-ATTACK-FLICKER` or `P1-EQUIP-TOOLTIP-BOUNDS`; keep `P1-COMBAT-POWER-EQUIPLESS-READOUT` in product-decision mode before changing any AS3 `combatPower` formula.
