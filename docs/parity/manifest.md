...
| P0-START-BURN | P0 | 创建/重生初始状态与自动存档 | Guarded | `p0-start-burn-save.md` | `Player.as`, `RaceScene.as` | `Player.ts`, `GameContext.tsx`, `SaveSystem.ts`, `actions.ts` | 创建/重生初始装备、12 个初始技能和创建后即时存档已由 guard 覆盖；Next: browser smoke | Existing: `assert:start-burn-save`; Adjacent: `assert:growth-skill-protection`, `assert:equipment-ownership`, `assert:save-persistence`; Always: `npx tsc -b` |
...
| P0-START-BURN | P0 | Creation/rebirth startup state and auto-save | Guarded | `p0-start-burn-save.md` | `Player.as`, `RaceScene.as` | `Player.ts`, `GameContext.tsx`, `SaveSystem.ts`, `actions.ts` | Creation/rebirth starter equipment, 12 starter skills, and immediate save after creation are covered by a guard; Next: browser smoke | Existing: `assert:start-burn-save`; Adjacent: `assert:growth-skill-protection`, `assert:equipment-ownership`, `assert:save-persistence`; Always: `npx tsc -b` |
