...
| `P1-ITEM-WINDOW-DETAIL` | P1 | AS3 parity | 背包选中装备强化时右侧多出品质/等级 UI 栏 | Verified | `assert:forge-ui-placement` (existing) |
| `P0-BATTLE-INIT-HEAL` | P0 | AS3 parity (confirmed original) | 每只怪物战斗后恢复主角和宠物血量 | Verified (no repair) | AS3 `Battle.init()` evidence; no new guard |
...
| `P2-BATTLE-LOG-CAPACITY` | P2 | Intentional divergence | 战斗日志历史条目可以增加 1.5x 容量 | Guarded | `assert:battle-log-sticky-scroll` (existing, 修改常量) |
| `P1-PLAYER-INFO-DISPLAY` | P1 | AS3 parity | 角色状态栏显示效果与 AS3 不一致（颜色、格式、攻击范围） | Verified | `assert:stat-list` (expanded) |
...
**Current Status:** Verified on 2026-06-04. `ItemWindow` 已移除 AS3 中不存在的选中装备品质/等级详情面板，锻造预览、成功率、费用、自动锻造、音效和锻造按钮仍保留在 `data-bwe-forge-panel="inventory-lower"` 下方面板；`assert:forge-ui-placement` 和 `assert:equip-window` 已更新为防回归 guard，浏览器 smoke 已确认上方背包列表区不含 `品质:` / `等级:`，下方锻造区仍显示成功率、费用和锻造按钮。
...
**Current Status:** Verified (no repair) on 2026-06-04. 经 AS3 与 React 对照确认，击杀一只怪物后 `Battle.init()` 重新读取最大 HP/MP 并回满是 AS3 原作行为；本轮按用户确认不修改战斗逻辑、不新增 guard。若后续观察到同一只怪物战斗过程中非战斗逻辑回血，应另开新卡排查。
...
**确认结论:** 用户已确认本卡所述击杀后回满属于原版实现，本卡关闭为无需修复。
...
**Red Guard Contract:** 无需新增。若未来重开本卡，应先新增 guard 检测同一怪物战斗过程中 `battle.playerHp` 是否被非战斗逻辑修改。

**Repair Direction:** 无需修复；保留 React `Battle.init()` 与 AS3 `Battle.init()` 的击杀后回满行为。

**Acceptance Tests:** 本卡关闭无需专属 acceptance；相邻战斗回归仍由 `assert:battle-damage-log-death`、`assert:battle-player-state` 和 `npx tsc -b` 覆盖。
...
**Current Status:** Guarded on 2026-06-04. React `MAX_BATTLE_LOG_MESSAGES` 已从 AS3 baseline 的 100 条提高到 150 条，并由 `assert:battle-log-sticky-scroll` 同时确认 AS3 原始 100 条基线与 React 的 intentional divergence 容量。
...
**Current Status:** Verified on 2026-06-04. `PlayerInfoPanel` 已按 AS3 恢复攻击范围 `attMin~attMax`、五项主属性相对 `basic*` 的红/金着色与基础值括号、暴击倍数百分比显示；`assert:stat-list` 已扩展覆盖这些显示契约，浏览器 smoke 已确认左上状态栏可见展示正确。
...
- `src/components/panels/playerInfoDisplay.ts`
...
**Acceptance Tests:** `npm run assert:stat-list`, `npm run assert:equip-window`, `npx tsc -b`
...
**Manual Smoke:** 进入游戏，观察左上角状态栏：攻击力显示范围（如 62~129）、属性值显示带颜色+基础值括号（如红色 `13(15)` 或金色 `17(15)`）、暴击倍数显示为百分比；2026-06-04 浏览器 smoke 已通过。
...
| `P1-ITEM-WINDOW-DETAIL` | P1 | AS3 parity | Extra quality/level UI bar appears when selecting equipment for forging in bag | Verified | `assert:forge-ui-placement` (existing) |
| `P0-BATTLE-INIT-HEAL` | P0 | AS3 parity (confirmed original) | Player and pet HP/MP recover after each monster kill | Verified (no repair) | AS3 `Battle.init()` evidence; no new guard |
...
| `P2-BATTLE-LOG-CAPACITY` | P2 | Intentional divergence | Battle log history can increase to 1.5x capacity | Guarded | `assert:battle-log-sticky-scroll` (existing, update constant) |
| `P1-PLAYER-INFO-DISPLAY` | P1 | AS3 parity | Character status bar display differs from AS3 (colors, format, attack range) | Verified | `assert:stat-list` (expanded) |
...
3. `P1-PLAYER-INFO-DISPLAY` — 已修复并通过 guard/smoke
4. `P1-ITEM-WINDOW-DETAIL` — 多余 UI 干扰
5. `P0-BATTLE-INIT-HEAL` — 已确认是 AS3 原作行为，无需修复
