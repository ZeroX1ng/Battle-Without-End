...
**Scope:** 测试阶段临时增加游戏倍率按钮和“无敌”（一击必杀效果）开关，方便快速推进进度。

**Parity Note:** 这是临时测试工具，不进入正式 parity 完成定义。实现必须有单一常量或 feature flag，便于发布前整卡移除；正式版发布前需要删除倍率控件、“无敌”开关和对应 `BattleDebugOptions` 链路。
...
**Red Guard Contract:** 新增/扩展 `assert:test-speed-control`，确认倍率选项为 `1x/10x/25x/50x`，只影响 `useGameLoop` 的 effective interval；“无敌”默认关闭，使用函数式切换保证打开后可关闭，只在开启时通过 `BATTLE_TICK` meta 传入当前 `Battle.run()`，不写入存档。

**Expected Behavior:** 测试控件可在主界面切换倍率和“无敌”；倍率越高逻辑 tick 越快；“无敌”打开后玩家回合会让当前怪物 HP 归零，便于验证 boss/宠物/掉落等长周期流程；再次点击“无敌”应关闭该效果；刷新或读档后倍率回到 1x、“无敌”回到关闭；生产移除时只需要删除一个 feature flag 分支、对应控件和 transient debug option。

**Forbidden Behavior:** 不允许把倍率或“无敌”序列化进 save；不允许倍率改变视觉 FPS cap 常量；不允许默认打开非 1x 或“无敌”；不允许把“无敌”做进 `GlobalConfig` 或 `GameState`。
...
**Manual Smoke:** 在主场景切换 1x/10x/25x/50x，观察战斗日志和年龄成长进度加速；打开“无敌”后确认当前怪物在玩家回合被击杀，再次点击确认 `aria-pressed` 从 `true` 回到 `false`；刷新后确认倍率回到 1x、“无敌”回到关闭。

**Current Status:** Verified. `MainScene` renders a feature-flagged temporary speed control with `1x/10x/25x/50x` plus a compact “无敌” button, derives only the `useGameLoop` interval from the selected multiplier, passes the one-hit debug option only through transient battle meta while enabled, defaults to `1x`/off, and leaves save/config state untouched. 2026-06-04 smoke confirmed the button toggles `false -> true -> false`.
...
| `P2-TEST-SPEED-CONTROL` | P2 | Temporary test tool | 13 | Verified | `assert:test-speed-control` | `1x/10x/25x/50x` debug speed affects loop interval only; the compact Invincible button toggles one-hit debug meta only while enabled; neither is saved. |
