...
- 2026-06-21: `assert:test-speed-control` now guards the player-name-side trigger, on-demand popover, release gating, and save-state boundary.
- 2026-06-21 browser smoke: trigger rect `{ x: 61.97, y: 20, width: 38, height: 22 }`; popover rect `{ x: 61.97, y: 46, width: 160, height: 64 }`; right tab rail rect stayed `{ x: 840.45, y: 11, width: 428.55, height: 44 }` after closing.
...
- 2026-06-21: Passed `assert:test-speed-control`, `assert:game-loop`, `assert:responsive-layout`, and `npx tsc -b`; browser smoke confirmed the trigger/popover stay left of the right tab rail and closing the popover leaves no right-rail placeholder.
