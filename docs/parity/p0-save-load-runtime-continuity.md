...
当前 React 仍有这个问题。读取存档后，玩家选择的非人类种族会回退为“人类”；同时存档里的日志和物品过滤开关会从 `true` 读成 `false`，导致战斗日志被 `shouldDisplayLog()` 过滤，装备掉落被 `shouldKeepDroppedItem()` 拒绝，挂机战斗看起来不再获得装备收益。

React 目前也没有按 AS3 语义把每 60 次战斗 tick 自动保存回当前存档槽。`BATTLE_TICK` 虽然有保存分支，但写入隐藏的 `auto` 槽；`GameState` 没有记录当前 `SaveScene.slot`，所以玩家在 `slot1`/`slot2`/`slot3` 中读取或新建后，原槽不会像 AS3 一样自动刷新。
...
- 当前 React `SaveSystem.deserializeSave()` 用 `RaceList.find(r => r.name === raceName) ?? RaceList[0]`，当中文 race name 经当前 Base64 往返变成损坏字符时会静默回退到“人类”。
- 当前 React `SaveSystem.deserializeSave()` 读取 toggle 时只接受 `val === '1'`，但 `serializeSave()` 写出的是 `true` / `false`，因此读取后 `battle_toggle`、`battleIntro_toggle`、`item_toggle`、`item1_toggle` 等会变成 `false`。
- 当前 React `GameContext.tsx` `BATTLE_TICK` 在 `result.shouldSave` 时写入 `localSave(..., 'auto', ...)`，不是当前读取/新建的槽位。
...
Current React still has this issue. After loading a save, a non-human selected race falls back to Human. Saved log and item filter toggles also round-trip from `true` to `false`, so `shouldDisplayLog()` hides battle logs and `shouldKeepDroppedItem()` rejects equipment drops. Idle battle therefore appears to stop producing equipment rewards after load.

React also does not match the AS3 current-slot auto-save behavior. `BATTLE_TICK` has a save branch, but it writes to a hidden `auto` slot. `GameState` does not remember the active `SaveScene.slot`, so loading or creating `slot1`/`slot2`/`slot3` does not auto-refresh that same slot like AS3 does.
